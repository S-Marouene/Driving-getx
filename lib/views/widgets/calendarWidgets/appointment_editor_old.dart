  ...
  void _updateAppointmentProperties() {
    if (widget.selectedAppointment != null) {
      ...
      _recurrenceProperties =
          widget.selectedAppointment!.recurrenceRule != null && widget.selectedAppointment!.recurrenceRule!.isNotEmpty
                              ...
                              builder: (BuildContext context) {
                                return WillPopScope(
                                    ...
                                    child: Theme(
                                      data: Theme.of(context),
                                      // ignore: prefer_const_literals_to_create_immutables
                                      child: _EditDialog(widget.model, newAppointment, widget.selectedAppointment!,
                                          _recurrenceProperties, widget.events),
                                    ));
                              });
                        } else {
...
List<String> _repeatOption = <String>['Never', 'Daily', 'Weekly', 'Monthly', 'Yearly'];

/// Dropdown list items for day of week
List<String> _weekDay = <String>['Lundi', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'];
...
List<String> _daysPosition = <String>['First', 'Second', 'Third', 'Fourth', 'Last'];
...
//// the given width agenda view will render the web UI.
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: calendarController.view == CalendarView.month && myModel.isWebFullView && screenHeight < 800
          ? Scrollbar(
  ...
  void _onCalendarTapped(CalendarTapDetails calendarTapDetails) {
    ...
    if (!myModel.isWebFullView && calendarController.view == CalendarView.month) {
      ...
      if (myModel.isWebFullView && !_isMobile) {
        final bool isAppointmentTapped = calendarTapDetails.targetElement == CalendarElement.appointment;
        showDialog<Widget>(
            context: context,
            builder: (BuildContext context) {
              final List<Appointment> appointment = <Appointment>[];
                ...
                child: Center(
                    child: Container(
                        ...
                        height: isAppointmentTapped
                            ? _selectedAppointment!.location == null || _selectedAppointment!.location!.isEmpty
                                ? 150
                                : 200
                            : 390,
                        child: Theme(
        ...
        Navigator.push<Widget>(
          ...
          MaterialPageRoute<Widget>(
              builder: (BuildContext context) => AppointmentEditor(myModel, _selectedAppointment, targetElement,
                  selectedDate, _colorCollection, _colorNames, _events, _timeZoneCollection)),
        );
      }
    }
  }
...
Widget _editExceptionSeries(BuildContext context, SampleModel myModel, Appointment selectedAppointment,
    Appointment recurrenceAppointment, CalendarDataSource events) {
  final Color defaultColor =
      Theme.of(context).colorScheme.brightness == Brightness.dark ? Colors.white : Colors.black54;
      ...
      child: ListView(padding: EdgeInsets.zero, children: <Widget>[
        ...
        Container(
            ...
            child: Text(
                ...
                style: TextStyle(fontSize: 15, fontWeight: FontWeight.w300, color: defaultTextColor))),
        Row(
          ...
          children: <Widget>[
            RawMaterialButton(
              ...
              fillColor: myModel.backgroundColor,
              onPressed: () {
                Navigator.pop(context);
                final List<DateTime>? exceptionDates = selectedAppointment.recurrenceExceptionDates;
                for (int i = 0; i < exceptionDates!.length; i++) {
                  final Appointment? changedOccurrence =
                      events.getOccurrenceAppointment(selectedAppointment, exceptionDates[i], '');
                  if (changedOccurrence != null) {
                    events.appointments!.removeAt(events.appointments!.indexOf(changedOccurrence));
                    events.notifyListeners(CalendarDataSourceAction.remove, <Appointment>[changedOccurrence]);
                  }
                }
                ...
                events.notifyListeners(CalendarDataSourceAction.add, <Appointment>[recurrenceAppointment]);
                Navigator.pop(context);
              },
              child: const Text(
                'YES',
            ...
            RawMaterialButton(
              onPressed: () {
                Navigator.pop(context);
                recurrenceAppointment.recurrenceExceptionDates = selectedAppointment.recurrenceExceptionDates;
                recurrenceAppointment.recurrenceRule = selectedAppointment.recurrenceRule;
                events.appointments!.removeAt(events.appointments!.indexOf(selectedAppointment));
                events.notifyListeners(CalendarDataSourceAction.remove, <Appointment>[selectedAppointment]);
                events.appointments!.add(recurrenceAppointment);
                events.notifyListeners(CalendarDataSourceAction.add, <Appointment>[recurrenceAppointment]);
...
Widget _deleteRecurrence(
    BuildContext context, SampleModel myModel, Appointment selectedAppointment, CalendarDataSource events) {
  final Color defaultColor =
      Theme.of(context).colorScheme.brightness == Brightness.dark ? Colors.white : Colors.black54;
  ...
  return Dialog(
    ...
    child: ListView(padding: EdgeInsets.zero, children: <Widget>[
      ...
      Container(
          ...
          height: 50,
          padding: const EdgeInsets.only(bottom: 10),
          child: Text('How would you like to change the appointment in the series?',
              style: TextStyle(fontSize: 15, fontWeight: FontWeight.w300, color: defaultTextColor))),
      Row(
        mainAxisAlignment: MainAxisAlignment.end,
        children: <Widget>[
          SizedBox(
            ...
            child: RawMaterialButton(
              ...
              onPressed: () {
                ...
                events.appointments!.removeAt(events.appointments!.indexOf(parentAppointment));
                events.notifyListeners(CalendarDataSourceAction.remove, <Appointment>[parentAppointment!]);
                parentAppointment.recurrenceExceptionDates != null
                    ? parentAppointment.recurrenceExceptionDates!.add(selectedAppointment.startTime)
                    : parentAppointment.recurrenceExceptionDates = <DateTime>[selectedAppointment.startTime];
                events.appointments!.add(parentAppointment);
                events.notifyListeners(CalendarDataSourceAction.add, <Appointment>[parentAppointment]);
                Navigator.pop(context);
              },
              child: const Text(
                'DELETE EVENT',
            ...
            child: RawMaterialButton(
              onPressed: () {
                ...
                if (parentAppointment!.recurrenceExceptionDates == null) {
                  ...
                  for (int i = 0; i < exceptionDates!.length; i++) {
                    final Appointment? changedOccurrence =
                        events.getOccurrenceAppointment(parentAppointment, exceptionDates[i], '');
                    if (changedOccurrence != null) {
...
Widget _editRecurrence(
  ...
  final List<Appointment> appointmentCollection = <Appointment>[];
  final Appointment? parentAppointment = events.getPatternAppointment(selectedAppointment, '') as Appointment?;
        ...
        children: <Widget>[
          RawMaterialButton(
            ...
            onPressed: () {
              ...
              selectedAppointment.id = selectedAppointment.appointmentType == AppointmentType.changedOccurrence
              ...
              showDialog<Widget>(
                  ...
                  builder: (BuildContext context) {
                    return WillPopScope(
                        ...
                        child: Theme(
                          data: Theme.of(context),
                          child: AppointmentEditorWeb(myModel, selectedAppointment, colorCollection, colorNames, events,
                              timeZoneCollection, appointmentCollection, visibleDates),
                        ));
                  });
            },
            child: const Text(
            ...
            showDialog<Widget>(
                ...
                builder: (BuildContext context) {
                  return WillPopScope(
                      ...
                      child: Theme(
                        data: Theme.of(context),
                        child: selectedAppointment.appointmentType == AppointmentType.normal
                            ...
                            : _editRecurrence(context, myModel, selectedAppointment, colorCollection, colorNames,
                                events, timeZoneCollection, visibleDates),
                      ));
                });
          },
        ),
              ...
              showDialog<Widget>(
                  ...
                  builder: (BuildContext context) {
                    return WillPopScope(
                        ...
                        child: Theme(
                          data: Theme.of(context),
                          child: _deleteRecurrence(context, myModel, selectedAppointment, events),
                        ));
                  });
            }
          },
...
class PopUpAppointmentEditor extends StatefulWidget {
  ...
  const PopUpAppointmentEditor(this.model, this.newAppointment, this.appointment, this.events, this.colorCollection,
      this.colorNames, this.selectedAppointment, this.timeZoneCollection, this.visibleDates,
      {super.key});
          ...
          child: ListTile(
            ...
            title: TextField(
              ...
              decoration: InputDecoration(
                ...
                focusedBorder:
                    UnderlineInputBorder(borderSide: BorderSide(color: widget.model.backgroundColor, width: 2.0)),
...
class AppointmentEditorWeb extends StatefulWidget {
  ...
  const AppointmentEditorWeb(this.model, this.selectedAppointment, this.colorCollection, this.colorNames, this.events,
      this.timeZoneCollection, this.appointment, this.visibleDates,
      [this.newAppointment]);
...
class _AppointmentEditorWebState extends State<AppointmentEditorWeb> {
  ...
  void _updateAppointmentProperties() {
    ...
    _recurrenceProperties =
        widget.selectedAppointment.recurrenceRule != null && widget.selectedAppointment.recurrenceRule!.isNotEmpty
            ? SfCalendar.parseRRule(widget.selectedAppointment.recurrenceRule!, _startDate)
            : null;
    _recurrenceProperties == null ? _neverRule() : _updateWebRecurrenceProperties();
  }

  void _updateWebRecurrenceProperties() {
    _recurrenceType = _recurrenceProperties!.recurrenceType;
    _week = _recurrenceProperties!.week;
    _weekNumber = _recurrenceProperties!.week == 0 ? _weekNumber : _recurrenceProperties!.week;
    _lastDay = _recurrenceProperties!.dayOfMonth;
    if (_lastDay != -1) {
      _dayOfMonth = _recurrenceProperties!.dayOfMonth == 1 ? _startDate.day : _recurrenceProperties!.dayOfMonth;
    }
    switch (_recurrenceType) {
      case RecurrenceType.daily:
        _dailyRule();
  ...
  void _endDateRange() {
    ...
    _selectedDate = _recurrenceProperties!.endDate ?? _startDate.add(const Duration(days: 30));
  ...
  void _countRange() {
    ...
    _count = _recurrenceProperties!.recurrenceCount == 0 ? 10 : _recurrenceProperties!.recurrenceCount;
  ...
  void _monthWeekIcon() {
    setState(() {
      ...
      _monthIconColor = Theme.of(context).colorScheme.brightness == Brightness.dark ? Colors.white : Colors.black54;
      _lastDayIconColor = Theme.of(context).colorScheme.brightness == Brightness.dark ? Colors.white : Colors.black54;
  ...
  void _monthDayIcon() {
    setState(() {
      ...
      _weekIconColor = Theme.of(context).colorScheme.brightness == Brightness.dark ? Colors.white : Colors.black54;
      _lastDayIconColor = Theme.of(context).colorScheme.brightness == Brightness.dark ? Colors.white : Colors.black54;
  ...
  Widget build(BuildContext context) {
    ...
    final Color defaultButtonColor =
        Theme.of(context).colorScheme.brightness == Brightness.dark ? Colors.white10 : Colors.white;
    final Color borderColor =
        Theme.of(context).colorScheme.brightness == Brightness.dark ? Colors.white : Colors.transparent;
    ...
    return Dialog(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          color: Theme.of(context).colorScheme.brightness == Brightness.dark ? Colors.grey[850] : Colors.white,
        ),
              ...
              child: ListView(
                ...
                children: <Widget>[
                  Container(
                      ...
                      child: ListTile(
                        title: Text(
                          widget.newAppointment == null ? 'Edit appointment' : 'New appointment',
                          style: TextStyle(fontSize: 18, fontWeight: FontWeight.w400, color: defaultTextColor),
                        ),
                        trailing: IconButton(
                          icon: Icon(Icons.close, color: defaultColor),
                                    ...
                                    TextField(
                                      ...
                                      decoration: InputDecoration(
                                        ...
                                        focusedBorder: UnderlineInputBorder(
                                            borderSide: BorderSide(color: widget.model.backgroundColor, width: 2.0)),
                                      ),
                                    ),
                                  ],
                                ),
                                ...
                                children: <Widget>[
                                  ...
                                  TextField(
                                    ...
                                    controller: TextEditingController(
                                        text: (_isAllDay ? DateFormat('MM/dd/yyyy') : DateFormat('MM/dd/yy h:mm a'))
                                            .format(_startDate)),
                                    onChanged: (String value) {
                                ...
                                child: Column(
                                    ...
                                    children: <Widget>[
                                      Text(
                                        'Repeat',
                                        style:
                                            TextStyle(fontSize: 12, color: defaultColor, fontWeight: FontWeight.w300),
                                        textAlign: TextAlign.start,
                                      ),
                                      TextField(
                                        mouseCursor: MaterialStateMouseCursor.clickable,
                                        controller: TextEditingController(text: _selectedRecurrenceType),
                                        decoration: InputDecoration(
                                          isDense: true,
                                          suffix: SizedBox(
                                              height: 28,
                                              child: DropdownButton<String>(
                                                  ...
                                                  items: _repeatOption.map((String item) {
                                      ...
                                      children: <Widget>[
                                        ...
                                        TextField(
                                          ...
                                          controller: TextEditingController.fromValue(TextEditingValue(
                                              ...
                                              selection: TextSelection.collapsed(offset: _interval.toString().length))),
                                          cursorColor: widget.model.backgroundColor,
                                          onChanged: (String value) {
                                            if (value.isNotEmpty) {
                                          ...
                                          maxLines: null,
                                          style: TextStyle(
                                              fontSize: 13, color: defaultTextColor, fontWeight: FontWeight.w400),
                                          decoration: InputDecoration(
                                            isDense: true,
                                            // isCollapsed: true,
                                            ...
                                            suffixIcon: Row(
                                              ...
                                              children: <Widget>[
                                                Padding(
                                                  ...
                                                  child: Material(
                                                    child: IconButton(
                                                        icon: Icon(Icons.arrow_drop_down_outlined, color: defaultColor),
                                            ...
                                            focusedBorder: UnderlineInputBorder(
                                                borderSide:
                                                    BorderSide(color: widget.model.backgroundColor, width: 2.0)),
                                          ),
                                        )
                                      ],
                                    ))),
                          ...
                          children: <Widget>[
                            Expanded(
                              ...
                              child: Visibility(
                                  ...
                                  child: Padding(
                                    padding: const EdgeInsets.only(left: 6, right: 5, top: 2, bottom: 2),
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: <Widget>[
                                        Text(
                                              ...
                                              children: <Widget>[
                                                Tooltip(
                                                  ...
                                                  child: ElevatedButton(
                                                    onPressed: () {
                                                      setState(() {
                                                        _webSelectWeekDays(WeekDays.sunday);
                                                      });
                                                    },
                                                ...
                                                Tooltip(
                                                  ...
                                                  child: ElevatedButton(
                                                    onPressed: () {
                                                      setState(() {
                                                        _webSelectWeekDays(WeekDays.monday);
                                                      });
                                                    },
                                                    style: ElevatedButton.styleFrom(
                                                      disabledForegroundColor: Colors.black26,
                                                ...
                                                Tooltip(
                                                  ...
                                                  child: ElevatedButton(
                                                    onPressed: () {
                                                      setState(() {
                                                        _webSelectWeekDays(WeekDays.tuesday);
                                                      });
                                                    },
                                                    style: ElevatedButton.styleFrom(
                                                      disabledForegroundColor: Colors.black26,
                                                ...
                                                Tooltip(
                                                  ...
                                                  child: ElevatedButton(
                                                    onPressed: () {
                                                      setState(() {
                                                        _webSelectWeekDays(WeekDays.wednesday);
                                                      });
                                                    },
                                                    style: ElevatedButton.styleFrom(
                                                      disabledForegroundColor: Colors.black26,
                                                ...
                                                Tooltip(
                                                  ...
                                                  child: ElevatedButton(
                                                    onPressed: () {
                                                      setState(() {
                                                        _webSelectWeekDays(WeekDays.thursday);
                                                      });
                                                    },
                                                    style: ElevatedButton.styleFrom(
                                                      disabledForegroundColor: Colors.black26,
                                                ...
                                                Tooltip(
                                                  ...
                                                  child: ElevatedButton(
                                                    onPressed: () {
                                                      setState(() {
                                                        _webSelectWeekDays(WeekDays.friday);
                                                      });
                                                    },
                                                    style: ElevatedButton.styleFrom(
                                                      disabledForegroundColor: Colors.black26,
                                                ...
                                                Tooltip(
                                                  ...
                                                  child: ElevatedButton(
                                                    onPressed: () {
                                                      setState(() {
                                                        _webSelectWeekDays(WeekDays.saturday);
                                                      });
                                                    },
                                                    style: ElevatedButton.styleFrom(
                                                      disabledForegroundColor: Colors.black26,
                              ...
                              child: Visibility(
                                ...
                                child: Padding(
                                    ...
                                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: <Widget>[
                                      ...
                                      TextField(
                                        mouseCursor: MaterialStateMouseCursor.clickable,
                                        controller: TextEditingController(text: _monthName),
                                        decoration: InputDecoration(
                                          isDense: true,
                                          suffix: SizedBox(
                                            height: 27,
                                            child: DropdownButton<String>(
                                                ...
                                                style: TextStyle(
                                                    fontSize: 13, color: defaultTextColor, fontWeight: FontWeight.w400),
                                                value: _monthName,
                                                items: _dayMonths.map((String item) {
                                                  return DropdownMenuItem<String>(
                                                    value: item,
                                                    child: Text(item),
                                                  );
                                          ...
                                          child: Padding(
                                              ...
                                              child: TextField(
                                                ...
                                                onChanged: (String value) {
                                                  ...
                                                  _recurrenceProperties!.week = 0;
                                                  _recurrenceProperties!.dayOfMonth = _dayOfMonth;
                                                  _weekIconColor = Theme.of(context).brightness == Brightness.dark
                                                      ? Colors.white
                                                      : Colors.black54;
                                                  _monthIconColor = widget.model.backgroundColor;
                                                  _monthDayRadio = Icons.radio_button_checked;
                            ...
                            Expanded(
                              ...
                              child: Visibility(
                                  visible: _selectedRecurrenceType != 'Never',
                                  child: Container(
                                    padding: EdgeInsets.only(left: _padding, top: 2, bottom: 2),
                                    ...
                                    child: Column(
                                      ...
                                      children: <Widget>[
                                        Text('End',
                                            style: TextStyle(
                                                fontSize: 12, color: defaultColor, fontWeight: FontWeight.w300),
                                            textAlign: TextAlign.start),
                                        Padding(
                                            padding: EdgeInsets.only(top: _selectedRecurrenceType == 'Monthly' ? 9 : 0),
                                            child: Row(
                                                crossAxisAlignment: _selectedRecurrenceType == 'Monthly'
                                                    ? CrossAxisAlignment.center
                                                    : CrossAxisAlignment.start,
                                                mainAxisSize: _selectedRecurrenceType == 'Monthly'
                                                    ? MainAxisSize.max
                                                    : MainAxisSize.min,
                                                children: <Widget>[
                                                  Expanded(
                                                      ...
                                                      child: Padding(
                                                          ...
                                                          child: TextField(
                                                            mouseCursor: MaterialStateMouseCursor.clickable,
                                                            controller:
                                                                TextEditingController(text: _selectedRecurrenceRange),
                                                            decoration: InputDecoration(
                                                              isDense: true,
                                                              suffix: SizedBox(
                                                                height: 27,
                                                                child: DropdownButton<String>(
                                                                    ...
                                                                    style: TextStyle(
                                                                        ...
                                                                        color: defaultTextColor,
                                                                        fontWeight: FontWeight.w400),
                                                                    value: _selectedRecurrenceRange,
                                                                    items: _ends.map((String item) {
                                                                      return DropdownMenuItem<String>(
                                                                        value: item,
                                                                        child: Text(item),
                                                                      );
                                                  ...
                                                  Expanded(
                                                      flex: _selectedRecurrenceRange == 'Count' ? 3 : 0,
                                                      child: Visibility(
                                                        visible: _selectedRecurrenceRange == 'Count',
                                                        child: Padding(
                                                            ...
                                                            child: TextField(
                                                              ...
                                                              decoration: InputDecoration(
                                                                ...
                                                                contentPadding: EdgeInsets.only(
                                                                    top: _selectedRecurrenceType == 'Monthly' ? 13 : 18,
                                                                    bottom: 10),
                                                  ...
                                                  Expanded(
                                                      flex: _selectedRecurrenceRange == 'Until' ? 3 : 0,
                                                      child: Visibility(
                                                          visible: _selectedRecurrenceRange == 'Until',
                                                          child: Container(
                                                            padding: const EdgeInsets.only(left: 9),
                                                            child: TextField(
                                                              ...
                                                              decoration: InputDecoration(
                                                                isDense: true,
                                                                contentPadding: EdgeInsets.only(
                                                                    top: _selectedRecurrenceType == 'Monthly' ? 10 : 15,
                                                                    bottom:
                                                                        _selectedRecurrenceType == 'Monthly' ? 10 : 13),
                                                                suffixIconConstraints:
                                                                    const BoxConstraints(maxHeight: 30),
                                                  ...
                                                  Spacer(
                                                      flex: _selectedRecurrenceType == 'Daily'
                                                          ? _selectedRecurrenceRange == 'Never'
                                                              ? 8
                                                              : 6
                                                          : _selectedRecurrenceRange == 'Never'
                        ...
                        Expanded(
                          ...
                          SizedBox(
                            ...
                            child: TextField(
                              controller: TextEditingController.fromValue(TextEditingValue(
                                  text: _dayOfMonth.toString(),
                                  selection: TextSelection.collapsed(offset: _dayOfMonth.toString().length))),
                              cursorColor: widget.model.backgroundColor,
                              onChanged: (String value) {
                                if (value.isNotEmpty) {
                                  _dayOfMonth = int.parse(value);
                                ...
                                _recurrenceProperties!.week = 0;
                                _recurrenceProperties!.dayOfMonth = _dayOfMonth;
                                _weekIconColor = Theme.of(context).colorScheme.brightness == Brightness.dark
                                    ? Colors.white
                                    : Colors.black54;
                                _monthIconColor = widget.model.backgroundColor;
                                _monthDayRadio = Icons.radio_button_checked;
                  ...
                  Visibility(
                      visible: _selectedRecurrenceType == 'Monthly' || _selectedRecurrenceType == 'Yearly',
                      child: Container(
                        padding: const EdgeInsets.only(top: 10),
                        width: 284,
                        height: 40,
                          ...
                          Row(
                            children: <Widget>[
                              Container(
                                ...
                                child: TextField(
                                  mouseCursor: MaterialStateMouseCursor.clickable,
                                  controller: TextEditingController(text: _weekNumberText),
                                  decoration: InputDecoration(
                                    isDense: true,
                                    suffix: SizedBox(
                                      height: 25,
                              ...
                              Container(
                                ...
                                child: TextField(
                                  mouseCursor: MaterialStateMouseCursor.clickable,
                                  controller: TextEditingController(text: _dayOfWeekText),
                                  decoration: InputDecoration(
                                    isDense: true,
                                    suffix: SizedBox(
                                      height: 25,
                    ...
                    children: <Widget>[
                      ...
                      Padding(
                        ...
                        child: RawMaterialButton(
                          ...
                          fillColor: widget.model.backgroundColor,
                          onPressed: () {
                            if ((widget.selectedAppointment.recurrenceRule != null &&
                                    widget.selectedAppointment.recurrenceRule!.isNotEmpty) ||
                                widget.selectedAppointment.recurrenceId != null) {
                              if (!_canAddRecurrenceAppointment(
                                  widget.visibleDates, widget.events, widget.selectedAppointment, _startDate)) {
                                showDialog<Widget>(
                                    context: context,
                                    builder: (BuildContext context) {
                                      return WillPopScope(
                            ...
                            if (widget.selectedAppointment.appointmentType != AppointmentType.normal &&
                                widget.selectedAppointment.recurrenceId == null) {
                              if (widget.selectedAppointment.recurrenceExceptionDates == null) {
                                widget.events.appointments!
                                    .removeAt(widget.events.appointments!.indexOf(widget.selectedAppointment));
                                widget.events.notifyListeners(
                                    CalendarDataSourceAction.remove, <Appointment>[widget.selectedAppointment]);
                                ...
                                widget.events
                                    .notifyListeners(CalendarDataSourceAction.add, <Appointment>[newAppointment]);
                                Navigator.pop(context);
                              } else {
                                ...
                                showDialog<Widget>(
                                    ...
                                    builder: (BuildContext context) {
                                      return WillPopScope(
                                          ...
                                          child: Theme(
                                              data: Theme.of(context),
                                              child: _editExceptionSeries(context, widget.model,
                                                  widget.selectedAppointment, recurrenceAppointment, widget.events)));
                                    });
                              }
                            } else if (widget.selectedAppointment.recurrenceId != null) {
                              final Appointment? parentAppointment =
                                  widget.events.getPatternAppointment(widget.selectedAppointment, '') as Appointment?;
                              widget.events.appointments!
                              ...
                              widget.events
                                  .notifyListeners(CalendarDataSourceAction.add, <Appointment>[parentAppointment]);
                              // ignore: unnecessary_null_comparison
                              if (widget.selectedAppointment != null || widget.newAppointment != null) {
                                if (widget.events.appointments!.isNotEmpty &&
                                    widget.events.appointments!.contains(widget.selectedAppointment)) {
...
class _SelectRuleDialog extends StatefulWidget {
  _SelectRuleDialog(this.model, this.recurrenceProperties, this.appointmentColor, this.events,
                  ...
                  onChanged: (_SelectRule? value) async {
                    final dynamic properties = await Navigator.push<dynamic>(
                      ...
                      MaterialPageRoute<dynamic>(
                          builder: (BuildContext context) => _CustomRule(widget.model, widget.selectedAppointment!,
                              widget.appointmentColor, widget.events, widget.recurrenceProperties)),
                    );
...
/// Dropdown list items for week number of the month.
List<String> _weekDayPosition = <String>['first', 'second', 'third', 'fourth', 'last'];

class _CustomRule extends StatefulWidget {
  const _CustomRule(
      this.model, this.selectedAppointment, this.appointmentColor, this.events, this.recurrenceProperties);
...
class _CustomRuleState extends State<_CustomRule> {
  ...
  void _updateAppointmentProperties() {
    ...
    _recurrenceProperties =
        widget.selectedAppointment.recurrenceRule != null && widget.selectedAppointment.recurrenceRule!.isNotEmpty
            ? SfCalendar.parseRRule(widget.selectedAppointment.recurrenceRule!, _firstDate)
            : null;
    _recurrenceProperties == null
  ...
  void _rangeCount() {
    _recurrenceProperties!.recurrenceRange = RecurrenceRange.count;
    _count = _recurrenceProperties!.recurrenceCount == 0 ? 1 : _recurrenceProperties!.recurrenceCount;
    _recurrenceProperties!.recurrenceCount = _count!;
  }
  ...
  double _textSize(String text) {
    const TextStyle textStyle = TextStyle(fontSize: 13, fontWeight: FontWeight.w400);
  ...
  Widget _getCustomRule(BuildContext context, Color backgroundColor, Color defaultColor) {
    final Color defaultTextColor =
        Theme.of(context).colorScheme.brightness == Brightness.dark ? Colors.white : Colors.black87;
    final Color defaultButtonColor =
        Theme.of(context).colorScheme.brightness == Brightness.dark ? Colors.white10 : Colors.white;
                            ...
                            ElevatedButton(
                              ...
                              style: ElevatedButton.styleFrom(
                                ...
                                backgroundColor: _days!.contains(WeekDays.sunday)
                                    ? widget.model.backgroundColor
                                    : defaultButtonColor,
                                foregroundColor: _days!.contains(WeekDays.sunday) ? Colors.white : defaultTextColor,
                                shape: const CircleBorder(),
                            ...
                            ElevatedButton(
                              ...
                              style: ElevatedButton.styleFrom(
                                ...
                                backgroundColor: _days!.contains(WeekDays.monday)
                                    ? widget.model.backgroundColor
                                    : defaultButtonColor,
                                foregroundColor: _days!.contains(WeekDays.monday) ? Colors.white : defaultTextColor,
                                shape: const CircleBorder(),
                            ...
                            ElevatedButton(
                              ...
                              style: ElevatedButton.styleFrom(
                                ...
                                backgroundColor: _days!.contains(WeekDays.tuesday)
                                    ? widget.model.backgroundColor
                                    : defaultButtonColor,
                                foregroundColor: _days!.contains(WeekDays.tuesday) ? Colors.white : defaultTextColor,
                                shape: const CircleBorder(),
                                padding: const EdgeInsets.all(12),
                              ),
                              child: const Text('T'),
                            ...
                            ElevatedButton(
                              ...
                              style: ElevatedButton.styleFrom(
                                ...
                                backgroundColor: _days!.contains(WeekDays.wednesday)
                                    ? widget.model.backgroundColor
                                    : defaultButtonColor,
                                foregroundColor: _days!.contains(WeekDays.wednesday) ? Colors.white : defaultTextColor,
                                shape: const CircleBorder(),
                                padding: const EdgeInsets.all(10),
                              ),
                              child: const Text('W'),
                            ...
                            ElevatedButton(
                              ...
                              style: ElevatedButton.styleFrom(
                                ...
                                backgroundColor: _days!.contains(WeekDays.thursday)
                                    ? widget.model.backgroundColor
                                    : defaultButtonColor,
                                foregroundColor: _days!.contains(WeekDays.thursday) ? Colors.white : defaultTextColor,
                                shape: const CircleBorder(),
                                padding: const EdgeInsets.all(12),
                              ),
                              child: const Text('T'),
                            ...
                            ElevatedButton(
                              ...
                              style: ElevatedButton.styleFrom(
                                ...
                                backgroundColor: _days!.contains(WeekDays.friday)
                                    ? widget.model.backgroundColor
                                    : defaultButtonColor,
                                foregroundColor: _days!.contains(WeekDays.friday) ? Colors.white : defaultTextColor,
                                shape: const CircleBorder(),
                                padding: const EdgeInsets.all(12),
                              ),
                              child: const Text('F'),
                            ...
                            ElevatedButton(
                              ...
                              style: ElevatedButton.styleFrom(
                                ...
                                backgroundColor: _days!.contains(WeekDays.saturday)
                                    ? widget.model.backgroundColor
                                    : defaultButtonColor,
                                foregroundColor: _days!.contains(WeekDays.saturday) ? Colors.white : defaultTextColor,
                                shape: const CircleBorder(),
                                padding: const EdgeInsets.all(12),
                              ),
                              child: const Text('S'),
                    ...
                    child: DropdownButton<String>(
                        ...
                        items: <DropdownMenuItem<String>>[
                          DropdownMenuItem<String>(
                            value: 'Monthly on day ' + _startDate.day.toString() + 'th',
                            child: Text('Monthly on day ' + _startDate.day.toString() + 'th'),
                          ),
                          DropdownMenuItem<String>(
                        ...
                        onChanged: (String? value) {
                          setState(() {
                            if (value == 'Monthly on day ' + _startDate.day.toString() + 'th') {
                              _width = _textSize('Monthly on day ' + _startDate.day.toString() + 'th');
                              _monthlyDay();
                            } else if (value == 'Monthly on the ' + _weekNumberDay!) {
                              _width = _textSize('Monthly on the ' + _weekNumberDay!);
                              _monthlyWeek();
                      ...
                      children: <Widget>[
                        ...
                        Container(
                          ...
                          child: TextField(
                            ...
                            controller: TextEditingController.fromValue(TextEditingValue(
                                text: _count.toString(),
                                selection: TextSelection.collapsed(offset: _count.toString().length))),
                            cursorColor: widget.model.backgroundColor,
                            onTap: () {
                              setState(() {
                                _endRule = _EndRule.count;
...
bool _canAddRecurrenceAppointment(
    List<DateTime> visibleDates, CalendarDataSource dataSource, Appointment occurrenceAppointment, DateTime startTime) {
  final Appointment parentAppointment = dataSource.getPatternAppointment(occurrenceAppointment, '')! as Appointment;
  final List<DateTime> recurrenceDates = SfCalendar.getRecurrenceDateTimeCollection(
      parentAppointment.recurrenceRule ?? '', parentAppointment.startTime,
      specificStartDate: visibleDates[0], specificEndDate: visibleDates[visibleDates.length - 1]);
  ...
  if (!_isDateInDateCollection(recurrenceDates, startTime)) {
    final int currentRecurrenceIndex = recurrenceDates.indexOf(occurrenceAppointment.startTime);
    if (currentRecurrenceIndex == 0 || currentRecurrenceIndex == recurrenceDates.length - 1) {
      ...
      canAddRecurrence = (isDateWithInDateRange(previousRecurrence, nextRecurrence, startTime) &&
